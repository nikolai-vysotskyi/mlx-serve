constexpr int TB = 32;
constexpr int DB = 16;                             // dv rows per threadgroup
...
const int dvr = tid / 16;            // 0..31
const int seg = tid % 16;            // 0..7
const int d0  = seg * 8;
...
float4 st[2];
{
    ...
    for (int i = 0; i < 2; ++i) st[i] = float4(S_in[i]);
}
...
for (int t0 = 0; t0 < T; t0 += TB) {
    ...
    for (int t = 0; t < tt; ++t) {
        ...
        float4 kf[2];
        for (int i = 0; i < 2; ++i) kf[i] = float4(k4[i]);
        ...
        const float kv_mem = simd_shuffle(part, (tid % 32) / 16 * 16);
...

{
    ...
    for (int i = 0; i < 2; ++i) S_out[i] = vec<StT,4>(st[i]);
}
