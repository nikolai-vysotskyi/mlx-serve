constexpr int TB = 32;
constexpr int DB = 8;                             // dv rows per threadgroup
...
const int dvr = tid / 32;            // 0..31
const int seg = tid % 32;            // 0..7
const int d0  = seg * 4;
...
float4 st[1];
{
    ...
    for (int i = 0; i < 1; ++i) st[i] = float4(S_in[i]);
}
...
for (int t0 = 0; t0 < T; t0 += TB) {
    ...
    for (int t = 0; t < tt; ++t) {
        ...
        float4 kf[1];
        for (int i = 0; i < 1; ++i) kf[i] = float4(k4[i]);
        ...
        part += simd_shuffle_down(part, 16);
        part += simd_shuffle_down(part, 8);
        part += simd_shuffle_down(part, 4);
        part += simd_shuffle_down(part, 2);
        part += simd_shuffle_down(part, 1);
        const float kv_mem = simd_shuffle(part, (tid % 32) / 32 * 32);
        ...
        out += simd_shuffle_down(out, 16);
        out += simd_shuffle_down(out, 8);
...

{
    ...
    for (int i = 0; i < 1; ++i) S_out[i] = vec<StT,4>(st[i]);
}
