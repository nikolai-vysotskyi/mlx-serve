...
int main(int argc,char**argv) {
  ...
  for(int variant:{0,32,164,0}) {
    int BM=variant%100,WM=variant==164?4:2,WN=variant==264?4:2;
    auto run=[&]() {
      if(!BM)return stock();
    ...
    for(int i=0;i<9;i++){auto t=std::chrono::steady_clock::now();auto r=run();mx::eval(r);ms.push_back(std::chrono::duration<double,std::milli>(std::chrono::steady_clock::now()-t).count());}
    std::sort(ms.begin(),ms.end());
    std::cout<<"{\"tokens\":"<<M/10<<",\"BM\":"<<BM<<",\"WM\":"<<WM<<",\"WN\":"<<WN<<",\"median_ms\":"<<ms[4]<<",\"max_diff\":"<<diff<<"}"<<std::endl;
