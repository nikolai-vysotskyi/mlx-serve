constexpr int TB = 32;
constexpr int DB = 64;                             // dv rows per threadgroup
...
float4 st[8];
{
    ...
    for (int i = 0; i < 8; ++i) st[i] = float4(S_in[i]);
}
...
for (int t0 = 0; t0 < T; t0 += TB) {
    ...
    for (int t = 0; t < tt; ++t) {
        ...
        float4 kf[8];
        for (int i = 0; i < 8; ++i) kf[i] = float4(k4[i]);
        ...
        const float kv_mem = simd_shuffle(part, (tid % 32) / 4 * 4);
...

{
    ...
    for (int i = 0; i < 8; ++i) S_out[i] = vec<StT,4>(st[i]);
}
