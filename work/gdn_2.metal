constexpr int TB = 16;
constexpr int DB = 128;                             // dv rows per threadgroup
...
const int dvr = tid / 2;            // 0..31
const int seg = tid % 2;            // 0..7
const int d0  = seg * 64;
...
float4 st[16];
{
    ...
    for (int i = 0; i < 16; ++i) st[i] = float4(S_in[i]);
}
...
for (int t0 = 0; t0 < T; t0 += TB) {
    ...
    for (int t = 0; t < tt; ++t) {
        ...
        float4 kf[16];
        for (int i = 0; i < 16; ++i) kf[i] = float4(k4[i]);
        ...
        const float kv_mem = simd_shuffle(part, (tid % 32) / 2 * 2);
...

{
    ...
    for (int i = 0; i < 16; ++i) S_out[i] = vec<StT,4>(st[i]);
}
