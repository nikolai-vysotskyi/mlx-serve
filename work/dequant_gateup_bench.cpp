...
int main(int argc,char**argv) {
  ...
  auto direct=mx::fast::metal_kernel("direct_gateup",{"x_input","w","scales","biases","indices_input","tiles","up_w","up_scales","up_biases","sigtab","lhs_indices"},{"out"},read_file("work/direct_gateup.metal"),read_file("work/grouped_qmm_header.metal"),true);
  for(int variant:{0,2164}) {
    int BM=variant%100,WM=variant%1000==164?4:2,WN=variant==264?4:2;
    auto run=[&]() {
      if(!BM)return stock();
      if(variant==2164) {
        auto x=mx::take(base,lhs_idx,0);
        auto dw=mx::transpose(mx::dequantize(w,sc,bi,64,4,"affine"),{0,2,1});
        auto du=mx::transpose(mx::dequantize(wu,usc,ubi,64,4,"affine"),{0,2,1});
        auto g=mx::gather_mm(x,dw,{},ind,true);
        auto u=mx::gather_mm(x,du,{},ind,true);
        return swiglu({g,u,tab,mx::array(M*N)},{{M,1,N}},{mx::bfloat16},{M*N,1,1},{256,1,1},{{"T",mx::bfloat16}},{},false,mx::Device::gpu)[0];
      }
