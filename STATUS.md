# STATUS — prefill optimization (GDN chunkwise + HC up+mix + MoE down+reduce)

- **Branch:** `arena/01a08be3-mlx-serve` (pushed to origin `nikolai-vysotskyi/mlx-serve`)
- **HEAD:** see `git log -1` (after each push); ancestry …62eac76 → 3fb11fc →
  f3067e0 → 4b15430 → d8f4b77 → (MoE down+reduce commit).
...
- **NOT built / run on Metal.** MSL compile + `zig build test` need Apple
  Silicon (CONTRIBUTING bar). All three kernels are UNBUILT; the GDN fold bugs
...
down+reduce (small). Remaining: MoE gate/up/GeGLU fusion (handoff
`patches/moe-gateup.patch`, NAX+steel, compiled-on-Mac but unproven),
attention/QSA reuse (~23%), HC write side, NAX perf pass for the up-mix GEMM.
None of the individual levers reaches 1.5× alone.
