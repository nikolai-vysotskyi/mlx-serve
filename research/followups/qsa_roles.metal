...
Bank bank;
if(role==0)bank.Q.load_rows(Qp,int(q_strides[1]),short(s<qL?gqa:0));
else bank.O.clear();
...
for(int it=0;it<NT;it++) {
  ...
  if(role==0) {
    ...
    if(active) {
      ...
      for(short d=0;d<16;d++) {
        ...
        BaseNAXFrag::mma(S.frag_at(0,0),S.frag_at(0,1),bank.Q.frag_at(0,d),
  ...
  if(role!=0) {
    ...
    bank.O.template row_bin_op<QsaMul>(factor);
    if(active) {
      STEEL_PRAGMA_UNROLL
      for(short ik=0;ik<2;ik++) {
        NAXTile<T,1,1> hi,lo;
        ...
        for(short d=0;d<8;d+=2) {
          ...
          BaseNAXFrag::mma(bank.O.frag_at(0,d),bank.O.frag_at(0,d+1),hi.frag_at(0,0),
            metal::false_type{},V.frag_at(0,0),V.frag_at(0,1),metal::false_type{});
          BaseNAXFrag::mma(bank.O.frag_at(0,d),bank.O.frag_at(0,d+1),lo.frag_at(0,0),
            metal::false_type{},V.frag_at(0,0),V.frag_at(0,1),metal::false_type{});
        }
      }
    }
...
if(role!=0) {
  ...
  bank.O.template row_bin_op<QsaMul>(inv);
  device T* Op=out+(((long)bb*Hq+hk*gqa)*qL+s)*BD+(role-1)*128;
  bank.O.store_rows(Op,qL*BD,short(s<qL?gqa:0));
}
