...
int main() {
  ...
  auto dd=mx::dequantize(dw,ds,db,64,4,"affine");
  auto ud=mx::dequantize(uw,us,ub,64,4,"affine");
  mx::eval(dd,ud);
  auto run=[&](int mode){
    ...
    auto down=(mode&4)?mx::matmul(mx::reshape(n,{M,K}),mx::transpose(dd)):mx::quantized_matmul(mx::reshape(n,{M,K}),dw,ds,db,true,64,4,"affine");
    auto act=silu({down})[0];
    auto up=mx::reshape((mode&4)?mx::matmul(act,mx::transpose(ud)):mx::quantized_matmul(act,uw,us,ub,true,64,4,"affine"),{M,HC,H});
