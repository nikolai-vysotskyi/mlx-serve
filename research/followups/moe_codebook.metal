    ...
    dispatch_bool(aligned_N || !is_unaligned_bn, [&](auto kAlignedN) {
      for (int k = 0; k < K; k += BK) {
        ...
        if constexpr (kAlignedN.value) {
          // Each pair of half-warps forms an exact BF16 16-entry codebook.
          STEEL_PRAGMA_UNROLL
          for(int pair=0;pair<8;pair++) {
            int row=simd_gid*16+pair*2+simd_lid/16;
            int col=simd_lid%16;
            float sc=float(scales[row*K_g+k/64]);
            float bi=float(biases[row*K_g+k/64]);
            ushort value=as_type<ushort>(T(sc*float(col)+bi));
            ushort codes=*((const device ushort*)(wl+row*K_w+k/2)+col);
            STEEL_PRAGMA_UNROLL
            for(int j=0;j<4;j++) {
              uint index=((simd_lid/16)*16)+((codes>>(4*j))&15);
              Ws[row*BK_padded+col*4+j]=as_type<T>(simd_shuffle(value,index));
            }
          }
        } else {
