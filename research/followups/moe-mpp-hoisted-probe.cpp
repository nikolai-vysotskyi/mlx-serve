...
int main(int argc,char**argv) {
  ...
  auto mpp=mx::fast::metal_kernel("moe_mpp_gateup",{"x_rep","w","scales","biases","indices_input","tiles","up_w","up_scales","up_biases","sigtab"},{"out"},read_file("research/followups/moe-mpp-gateup-hoisted.metal"),read_file("src/kernels/qsa_nax_header.metal"));
  auto old_mpp=mx::fast::metal_kernel("moe_mpp_gateup_old",{"x_rep","w","scales","biases","indices_input","tiles","up_w","up_scales","up_biases","sigtab"},{"out"},read_file("research/followups/moe-mpp-gateup.metal"),read_file("src/kernels/qsa_nax_header.metal"));
  auto run=[&](int mode) {
    ...
    if(mode==5 || mode==8) {
      ...
      a=(mode==8?mpp:old_mpp)({rep,g[0],g[1],g[2],sorted,p[3],u[0],u[1],u[2],tab},{{M,1,I}},{mx::bfloat16},{(I*2/128)*32,capacity*WM*2,1},{32,WM*2,1},{{"T",mx::bfloat16},{"BM",BM},{"WM",WM}},{},false,mx::Device::gpu)[0];
  ...
  for(int mode:std::vector<int>{1,5,8,5,1}) {
