...
constexpr int BN=64,BK=64,LD=72,SM=BM/WM,SN=32;
...
const int tm=(sg/2)*SM,tn=(sg%2)*32;
const int valid=min(SM,max(0,M-ym-tm));
...
NAXTile<float,SM/16,2> D;D.clear();
for(int k=0;k<K;k+=64) {
  ...
  for(int kk=0;kk<64;kk+=32) {
    ...
    // Keep one M fragment of A live while reusing B across a wider accumulator.
    STEEL_PRAGMA_UNROLL
    for(short im=0;im<SM/16;im++) {
      NAXTile<T,1,2> A;
      STEEL_PRAGMA_UNROLL
      for(short ak=0;ak<2;ak++) {
        ...
        for(short j=0;j<8;j++) {
          ...
          A.frag_at(0,ak)[j]=x_input[rowbase[im*2+j/4]+cc];
        }
      }
      STEEL_PRAGMA_UNROLL
      for(short ik=0;ik<2;ik++)
        BaseNAXFrag::mma(D.frag_at(im,0),D.frag_at(im,1),A.frag_at(0,ik),metal::false_type{},B.frag_at(0,ik),B.frag_at(1,ik),metal::true_type{});
    }
  }
}
const short2 coord=BaseNAXFrag::get_coord();
STEEL_PRAGMA_UNROLL
for(short fm=0;fm<SM/16;fm++) {
  STEEL_PRAGMA_UNROLL
