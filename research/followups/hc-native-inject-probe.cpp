...
int main(int argc,char**argv) {
  ...
  auto makew=[&](int n,int k){
    std::vector<uint32_t> data(size_t(n)*k/(32/bits));for(auto& z:data)z=rng();
    return mx::array(data.data(),{n,k/(32/bits)});
  };
  ...
  auto exact_nk=mx::fast::metal_kernel("hc_norm_native_inject",{"x","w","iw","eps","wo","wi"},{"normed","ipart","stream"},read_file("research/followups/hc-native-inject-norm.metal"),"",true);
  ...
  auto run=[&](int mode){
    ...
    if(mode&1) {
      auto p=((mode&4)?exact_nk:nk)({x,w,iw,eps,wo,wi},{{M,HC,H},{M,HC,HC},{M,HC,H}},{mx::bfloat16,mx::float32,mx::bfloat16},{M*HC*(H/4),1,1},{H/4,1,1},{{"T",mx::bfloat16},{"H",H},{"HC",HC},{"WR",true}},{},false,mx::Device::gpu);
      n=p[0];stream=p[2];raw=(mode&4)?mx::matmul(mx::reshape(n,{M,K}),iw):mx::astype(mx::sum(p[1],1),mx::bfloat16);
    } else {stream=write({x,wo,wi})[0];n=mx::fast::rms_norm(stream,ones,1e-6)*w;raw=mx::matmul(mx::reshape(n,{M,K}),iw);}
    auto down=mx::quantized_matmul(mx::reshape(n,{M,K}),dw,ds,db,true,64,bits,"affine");
    auto act=silu({down})[0];
    auto up=mx::reshape(mx::quantized_matmul(act,uw,us,ub,true,64,bits,"affine"),{M,HC,H});
  ...
  for(int mode:{0,3,7,0}) {
    auto y=run(mode);mx::eval(y);
    std::cout<<"{\"bits\":"<<bits<<",\"mode\":"<<mode<<",\"diffs\":[";
    for(int j=0;j<5;j++)std::cout<<(j?",":"")<<mx::max(mx::abs(mx::astype(y[j],mx::float32)-mx::astype(ref[j],mx::float32))).item<float>();
    std::cout<<"]";
    if(mode==7)for(int j=0;j<5;j++)if(!mx::all(y[j]==ref[j]).item<bool>())throw std::runtime_error("native-inject path must be bit exact");
    if(mode==3) {
