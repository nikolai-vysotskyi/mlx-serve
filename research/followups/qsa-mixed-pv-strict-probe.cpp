...
int main(int argc,char** argv) {
  ...
  auto header=read_file("research/followups/qsa_mixed_pv_strict_header.metal");
  ...
  for(int arm:{0,1,2,1,0}) {
    ...
    auto fn=mx::fast::metal_kernel("qsa_bucketed",{"q","k","v","scl","blocks","tilepos","tilemask"},{"out"},read_file(arm==2?"research/followups/qsa_pair_mixed_pv.metal":"src/kernels/qsa_pair.metal"),header,false);
    ...
    std::cout<<"{\"mixed_pv\":"<<(arm==2)<<",\"G\":"<<G<<",\"S\":"<<S<<",\"KV\":"<<KV<<",\"max_tiles\":"<<NT<<",\"tiles\":"<<work_tiles<<",\"active_tiles\":"<<active_tiles<<",\"ms\":"<<ts[1]<<",\"max_diff\":"<<diff<<",\"rmse\":"<<rmse<<",\"plan_included\":true}"<<std::endl;
  }
}
