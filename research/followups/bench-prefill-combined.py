...
records=[]
meta={'kind':('diagnostic; synchronization/trace overhead included' if args.trace or os.environ.get('PREFILL_CAPTURE_ROUTES')=='1' else 'HTTP matched screening; not llmprobe'),'binary_sha256':hashlib.sha256(BIN.read_bytes()).hexdigest(),'power':subprocess.check_output(['pmset','-g','batt'],text=True),'corpus_sha256':hashlib.sha256(corpus.encode()).hexdigest(),'records':records}
out=W/(TAG+'.json')
 ...
 for name,fused,packed in [('base',False,False),('current',True,False),('packed',True,True),('ahead',True,True),('combined',True,True),('group',True,False)]:
  ...
  with logpath.open('w') as log:
   ...
   try:
    ...
    for nonce,size in (cases[:1] if os.environ.get('PREFILL_CAPTURE_ROUTES')=='1' else cases):
