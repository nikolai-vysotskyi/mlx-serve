...
for(int it=0;it<=NT;it++) {
  const int mask=it<NT?masks[it]:0;
  const bool hascur=mask!=0;
  const bool active=s<qL && ((mask>>qi)&1);
  if(hascur)for(int i=tid;i<BK*32;i+=NSG*32) {
      ...
      for(short ik=0;ik<2;ik++) {
        ...
        for(short d=0;d<8;d+=2) {
          ...
          for(short c=0;c<2;c++) {
            STEEL_PRAGMA_UNROLL
            for(short j=0;j<2;j++) {
              int pos=poslist[(it-1)*BK+ik*16+coord.y+j*8];
              uint2 z=uint2(0);
              if(pos>=0 && pos<kL)z=*((const device uint2*)(Vp+(long)pos*v_strides[2]+(role-1)*128+(d+c)*16+coord.x));
  ...
  if(role==0 && hascur) {
    if(coord.x==0) {
      factors[qi*16+coord.y]=carry_factor.x;factors[qi*16+coord.y+8]=carry_factor.y;
