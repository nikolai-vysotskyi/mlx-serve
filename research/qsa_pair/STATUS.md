...
- Combined paired-QSA + HC integration: ReleaseFast build and two HTTP requests per arm passed, recorded in `../hc_prefill/http-smoke.json`. Initial warmed 15,715-token smoke: 1953.6 -> 2166.7 tok/s. With first-chunk routing corrected, final same-binary control: 1989.0 -> 2397.0 tok/s (+20.5%). This is not the 65K target or llmprobe evidence. Full suite remains pending; no PR.
...
First-chunk routing is implemented for supported paired-prefill geometry, with explicit threshold overrides and unsupported shapes retaining the old floor. ReleaseFast rebuild passed (7/7); HTTP on/off passed two shapes, and the on arm logged S8192/KV8192 without a threshold override. `first-chunk-validation.jsonl`: all 8192 rows have exact key multisets; 8192 output elements checked against float64, including the 2048-token budget transition; the new and stock maximum errors match. No full suite or long-prompt llmprobe was run.
