# Status — 2026-09-12

Base: fa76a4b50b3f54af7e9cd927279f5ba2870f02c6. Branch: perf/qwen-prefill-paired-qsa.

- Implemented GPU private/shared selection plan + paired NAX attention, opt-in server integration.
- ReleaseFast server build: passed on this M5 Max. Binary built with production changes; subsequent source change added/fixed only the focused regression test.
- Focused Zig regression: `zig build test -Dtest-filter="qsa pair" --summary all` passed, 9/9 build steps, 3/3 executed tests; the unrelated vz-agent tests were cached. This is **not** a full suite run.
- C++ float64 oracle: passed four shapes, 43,008 sampled output elements; selected-key multisets checked for **all** rows. Includes B2 and S8192, strided Q/K/V and partial tails.
- Captured-map attention microbenchmark: stock 29.5954/29.5001 ms, paired 14.1949 ms, GPU planning included; ~2.08× component speedup.
- Required long-prompt llmprobe comparison: **not measured on this branch**. 1868 tok/s is the user's fa76a4b baseline. Required >1.5× is >2802 tok/s, not 2500.
- Combined paired-QSA + HC integration: ReleaseFast build and two HTTP requests per arm passed, recorded in `../hc_prefill/http-smoke.json`. Initial warmed 15,715-token smoke: 1953.6 -> 2166.7 tok/s. With first-chunk routing corrected, final same-binary control: 1989.0 -> 2397.0 tok/s (+20.5%). This is not the 65K target or llmprobe evidence. The clean source copy is now draft PR #408; full suite passed (2349 passed, 154 skipped, 0 failed), recorded in `../pr-validation.json`.
- User requested few heavy tests: paired attention alone lacks the >=1.5× total-time budget, so no long full-model benchmark was launched.

Next meaningful work: find a second large arithmetic/data-layout lever in MoE/HC; the straightforward ideas already rejected here are pre-dequantized dense projections, larger generic/expert NAX tiles, and overlap of representative independent QSA+QMM stages. Keep the paired-QSA building block. Before any production/default-on change add its own startup JIT probe/fallback and include the metadata allocation in the admission bill. The production copy has passed the full suite and HTTP checks; reserve same-session llmprobe A/B for a credible combined candidate.

First-chunk routing is implemented for supported paired-prefill geometry, with explicit threshold overrides and unsupported shapes retaining the old floor. ReleaseFast rebuild passed (7/7); HTTP on/off passed two shapes, and the on arm logged S8192/KV8192 without a threshold override. `first-chunk-validation.jsonl`: all 8192 rows have exact key multisets; 8192 output elements checked against float64, including the 2048-token budget transition; the new and stock maximum errors match. The later clean production copy passed the full suite; long-prompt llmprobe remains pending.
