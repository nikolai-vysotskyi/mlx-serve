...
4. **GDN prefill prework and norm/gate fusion** extends the existing token-local kernels to B1–2 / 17–8192 total rows, including cold convolution history on the first chunk. Wide BF16 sigmoid uses a table generated by the native MLX operation, preserving its exact rounding; simply copying the sigmoid formula differed by one ULP in a new prefill test. See `gdn_prefill/README.md`.

All three kernel optimizations remain **opt-in**, not production defaults:

```sh
MLX_SERVE_QSA_PAIR=1 MLX_SERVE_HC_PREFILL=1 MLX_SERVE_GDN_PREFILL_FUSED=1 \
...
### Latest incremental GDN comparison

A new same-binary comparison holds paired QSA, HC and automatic first-chunk dispatch on, toggling only `MLX_SERVE_GDN_PREFILL_FUSED`:

| Prompt | GDN fusion off | GDN fusion on |
|---|---:|---:|
| 13,515 tokens, first long request | 2316.5 tok/s | 2380.1 tok/s |
| 15,715 tokens, second/warmed request | **2395.7 tok/s** | **2509.4 tok/s** |

That is **+4.75% incremental** on the warmed request. The on log confirms cold first-chunk GDN engagement. Every response recalled the passphrase with zero cached prompt tokens. `gdn_prefill/http-smoke.json` identifies the binary, arms and usage. This remains a short HTTP measurement, not the required long-context llmprobe result; do not use the historical 1868 number as its matched control.
...
Settings, model/header and runtime fingerprints, raw reports, engagement excerpts and the portable reproduction script are in `long_context/`. The recommended mixed model was tested in off/on/off order following the maintainer request in PR #375. All arms used explicit chunk8192, context131072, prefix cache entries0, KV quant off, MTP default off and PLD requested on but disabled by the Qwen4 scheduler. The standard llmprobe nonce produces a small documented token-count mismatch; prompt bytes are not identical across boots.

The additional short-query bridge and source/log discrepancy checks are in `long_context/README.md`. Client timing overhead is about 0.2%; PLD is actually disabled by Qwen4 module wiring. Neither explains the long-context gap. The reviewer’s 2083.1 tok/s still needs matching prompt and launch parameters.
...
python3 tests/test_qsa_pair_prefill.py --url http://127.0.0.1:11234 \
  --log /path/to/that-server.log --pair on --hc on --gdn on --first-chunk
```
